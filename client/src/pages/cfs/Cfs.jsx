...
const getAnonymousAvatar = (author) => {
  ...
  const [background, accent, detail] = anonymousAvatarStyles[seed % anonymousAvatarStyles.length];
  const mouth = ["M23 42c5 5 13 5 18 0", "M23 44c5-4 13-4 18 0", "M25 42h14"][seed % 3];
...
const Cfs = () => {
  ...
  const likeReply = async (postId, reply) => {
    ...
    const updateReply = (post, nextReply) => post?._id === postId ? { ...post, replies: post.replies.map((item) => item._id === reply._id ? { ...item, ...nextReply } : item) } : post;
    const optimisticReply = { liked: !reply.liked, likes: Math.max(0, reply.likes + (reply.liked ? -1 : 1)) };

    setPosts((current) => current.map((post) => updateReply(post, optimisticReply)));
    ...
    try {
      ...
      setPosts((current) => current.map((post) => updateReply(post, serverReply)));
  ...
  const sendReply = async () => { if (replySubmitting || !replyContent.trim() || !postId) return; try { setReplySubmitting(true); await createCfsReply(postId, { content: replyContent, isAnonymous: replyAnonymous, parentReplyId: replyingTo?.parentReplyId || null }); setReplyingTo(null); setReplyContent(""); setReplyAnonymous(false); const response = await getCfsPost(postId); setDetailPost(response.data?.data?.post || null); await loadPosts(page); } catch (error) { message.error(error.response?.data?.message || "Không thể gửi phản hồi"); } finally { setReplySubmitting(false); } };
  const setDetailPostId = (id) => navigate(`/cfs/${id}`);
  if (postId) return <DetailPage post={detailPost} onClose={() => navigate("/cfs")} admin={user?.role === "admin"} onlineUsers={onlineUsers} viewer={user} onLike={like} onDeletePost={removePost} onDeleteReply={removeReply} onLikeReply={likeReply} replyingTo={replyingTo} setReplyingTo={setReplyingTo} replyContent={replyContent} setReplyContent={setReplyContent} replyAnonymous={replyAnonymous} setReplyAnonymous={setReplyAnonymous} alias={alias} requestAnonymous={requestAnonymous} onSendReply={sendReply} replySubmitting={replySubmitting} replySort={replySort} setReplySort={setReplySort} />;
